    ...
    public partial class WebForm1x : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["type"] != null)
            {
                ...
                if (get == "5")
                    CurrentWordTable = AllWordsIAddedTable();
                if (get == "6")
            ...
            {
                Response.Redirect("../english/self-dictionary.aspx");
            }
        ...
        public DataTable AllWordsTable()
        {
            ...
            {
                c.conOpen();
                MySqlCommand msc = new MySqlCommand("SELECT * FROM words WHERE `username_id` = " + Autorization.SessionId + " AND `language` = 'english'", c.Con);
        ...
        public DataTable AllLikedWordsTable()
        {
            ...
            {
                c.conOpen();
                MySqlCommand msc = new MySqlCommand("SELECT * FROM words WHERE `username_id` = " + Autorization.SessionId + " AND `liked_id` != 0 AND `language` = 'english'", c.Con);
                DataSet ds = c.getDataSet(msc, "words");
                dt = ds.Tables["words"];
            }
            finally
        ...
        private static DataTable GetWordsByType(string table, int type)
        {
            ...
            {
                c.conOpen();
                MySqlCommand msc = new MySqlCommand("SELECT * FROM " + table + " WHERE `username_id` = @username_id AND `knowledge` = @knowledge AND `language` = 'english'", c.Con);
        ...
        private static DataTable GetWordsByCategory(string table, int categoryId)
        {
            ...
            {
                c.conOpen();
                MySqlCommand msc = new MySqlCommand("SELECT * FROM " + table + " WHERE `username_id` = @username_id AND `category_id` = @category_id AND `language` = 'english'", c.Con);
                msc.Parameters.AddWithValue("@username_id", Autorization.SessionId);
