...
WHERE w.word = @type AND ass.liked_Id = 0 ORDER BY a.liked_id DESC LIMIT 0, 50",
    c.Con);
        ...
        public DataTable GetLast10Words(string table)
        {
            ...
            {
                c.conOpen();
                MySqlCommand msc = new MySqlCommand("SELECT * FROM words WHERE username_id = @username_Id AND `language` = 'hebrew' ORDER BY id DESC LIMIT 10;", c.Con);
        ...
        protected void SaveSettings_ServerClick(object sender, EventArgs e)
        {
            ...
            cookie.Expires = DateTime.UtcNow.AddDays(30);
            HttpContext.Current.Response.Cookies.Add(cookie);
            Response.Redirect("../hebrew/add-word.aspx");
        }
