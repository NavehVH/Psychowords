        ...
        public static string[][] GetData(int optionsNum, string category)
        {
            ...
            try
            {
                ...
                if (category == "o1")
                    ...
                    msc = new MySqlCommand("SELECT SQL_NO_CACHE * FROM `words` WHERE username_id = @username_id AND knowledge = 3 AND `language` = 'hebrew' ORDER BY RAND() LIMIT @limit");
                else if (category == "o3")
                    msc = new MySqlCommand("SELECT SQL_NO_CACHE * FROM `words` WHERE username_id = @username_id AND knowledge = 2 AND `language` = 'hebrew' ORDER BY RAND() LIMIT @limit");
                else if (category == "o4")
                    msc = new MySqlCommand("SELECT SQL_NO_CACHE * FROM `words` WHERE username_id = @username_id AND knowledge = 1 AND `language` = 'hebrew' ORDER BY RAND() LIMIT @limit");
                else if (category == "o5")
                    msc = new MySqlCommand("SELECT SQL_NO_CACHE * FROM `words` WHERE username_id = @username_id AND knowledge = 0 AND `language` = 'hebrew' ORDER BY RAND() LIMIT @limit");
                else
                {
                    msc = new MySqlCommand("SELECT SQL_NO_CACHE * FROM `words` WHERE username_id = @username_id AND category_id = @category_id AND `language` = 'hebrew' ORDER BY RAND() LIMIT @limit");
