...
namespace Psychometric.master_pages
{
    public partial class WebForm4 : System.Web.UI.Page
    {
        ...
        public int GetCategoryCount(int categoryId)
        {
            ...
            {
                con.conOpen();
                MySqlCommand msc = new MySqlCommand("SELECT COUNT(*) FROM `words` WHERE category_id = @category_id AND username_id = @username_id AND `language` = 'hebrew'");
...
SELECT COUNT(*) FROM `words` WHERE username_id = " + id + @" AND `language` = 'hebrew';
SELECT COUNT(*) FROM `words` WHERE username_id = " + id + @" AND `knowledge` = 3 AND `language` = 'hebrew';
SELECT COUNT(*) FROM `words` WHERE username_id = " + id + @" AND `knowledge` = 2 AND `language` = 'hebrew';
SELECT COUNT(*) FROM `words` WHERE username_id = " + id + @" AND `knowledge` = 1 AND `language` = 'hebrew';
SELECT COUNT(*) FROM `words` WHERE username_id = " + id + @" AND `liked_id` != 0 AND `language` = 'hebrew';
");
                msc.Connection = con.Con;
                MySqlDataReader r = msc.ExecuteReader();
                if (r.Read())
